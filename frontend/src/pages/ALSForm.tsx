...
export default function ALSForm() {
  ...
  const handleSaveReport = async () => {
    ...
    setSaveError(null);
    try {
      const res = await fetch('http://localhost:5001/api/patients/save-report', {
        method: 'POST',
        headers: {
          ...
          <div className="panel">
            <div className="panel-head">
              ...
              <div>
                ...
                <div className="panel-sub">XGBoost model prediction output</div>
              </div>
            </div>
            ...
            <div className="scroll">
              {!analyzed && !loading && (
                <div className="empty">
                  ...
                  <div className="empty-desc">
                    Enter patient data and run the model to see ALS risk prediction
                  </div>
                </div>
              )}
              ...
              {analyzed && result && !loading && riskLevel && rc && (
                <div className="fadein">
                  ...
                  <div className="divider" />
                  {saveSuccess ? (
                    <div style={{ background: 'rgba(0,255,170,0.08)', border: '1px solid rgba(0,255,170,0.3)', borderRadius: '10px', padding: '14px 16px', textAlign: 'center', color: '#00ffaa', fontSize: '13px' }}>
                      ✅ Report saved & emailed to your registered email!{' '}
                      <button onClick={() => navigate('/patient-history')} style={{ color: '#00c8ff', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontSize: '13px' }}>
                        View History →
                      </button>
                    </div>
                  ) : (
                    <>
                      {saveError && (
                        <div style={{ background: 'rgba(255,50,80,0.08)', border: '1px solid rgba(255,50,80,0.3)', borderRadius: '8px', padding: '10px 14px', color: '#ff3250', fontSize: '12px', marginBottom: '10px' }}>
                          ⚠ {saveError}
                        </div>
                      )}
                      <button
                      ...
                      >
                        {saving ? (
                          <><div className="spinner" /> Saving & Sending Email...</>
                        ) : (
